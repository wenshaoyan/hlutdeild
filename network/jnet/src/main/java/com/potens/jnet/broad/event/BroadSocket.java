...
import com.potens.jnet.broad.runnable.UDPRunnable;
...
//             localIp = String.valueOf(new Date().getTime());
    ...
    private void initWorkRole() {
        startRoleDecisionTimer();
        new Thread(new UDPRunnable(this.mus, new EventListener() {
            @Override
    ...
    private void send(byte event, byte[] bytes) {
        try {
            byte[] data = new byte[EventListener.MESSAGE_TOOL_BYTE];
            data[0] = event;
    ...
    public void sendJoinEvent() {
        send(EventListener.EVENT_W_JOIN, localIpByte);
    }

    // 由server发出的 server的地址
    public void sendServerAddress() {
        send(EventListener.EVENT_S_MANAGE_ADDRESS, localIpByte);
    }
    ...
    public void sendRoleChain() {
        ...
        send(EventListener.EVENT_S_SYNC_ROLE_CHAIN, bytes);
    }

    // 由server发出 有client退出
    public void sendClientExit(String ip) {
        byte[] bytes = ip.getBytes();
        send(EventListener.EVENT_S_CLIENT_EXIT, bytes);
    }

    // 由client发出 连接server断开
    public void sendServerDisconnect() {
        byte[] bytes = new byte[]{1};
        send(EventListener.EVENT_C_SERVER_DISCONNECT, bytes);

    }
    // 由client发出 同意加入
    public void sendConsentJoin() {
        send(EventListener.EVENT_C_CONSENT_JOIN, localIpByte);
